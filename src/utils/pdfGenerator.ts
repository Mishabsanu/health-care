...
export const generateInvoicePDF = (data: InvoiceData) => {
  ...
  doc.setTextColor(...textMain);
  doc.setFontSize(10);
  doc.text((data.clinicName || 'PHYSIO CLINIC').toUpperCase(), 20, startY + 6);
  doc.setTextColor(...textMuted);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  const splitAddress = doc.splitTextToSize(data.clinicAddress || 'Main Clinical Registry Site', 65);
  doc.text(splitAddress, 20, startY + 11);
  doc.text(`Contact: ${data.clinicPhone || 'N/A'}`, 20, startY + 20);
