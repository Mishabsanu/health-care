...
export default function PatientDetailsPage() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'appointments' | 'billing' | 'docs'>('overview');
    ...
    const fetchPatient = async () => {
        setLoading(true);
        try {
            const [ptRes, invRes, apptRes] = await Promise.all([
                api.get(`/patients/${id}`),
                api.get(`/invoices?search=${id}`),
                api.get(`/appointments?search=${id}`)
            ]);
            setPatient(ptRes.data);

            // Clinical Filtering | Invoices
            let invoiceList = invRes.data?.data || invRes.data;
            if (!Array.isArray(invoiceList)) invoiceList = [];
            const ptInvoices = invoiceList.filter((inv: any) => inv.patientId?._id === id || inv.patientId === id);
            setInvoices(ptInvoices);

            // Clinical Filtering | Appointments
            let apptList = apptRes.data?.data || apptRes.data;
            if (!Array.isArray(apptList)) apptList = [];
            const ptAppts = apptList.filter((appt: any) => appt.patientId?._id === id || appt.patientId === id);
            setAppointments(ptAppts);
        } catch (err) {
    ...
    <div className="patient-pro-workspace animate-fade-in" style={{ paddingBottom: '5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', borderBottom: '2px solid var(--border-subtle)' }}>
            {[
                ...
                { id: 'appointments', label: 'APPOINTMENTS', icon: ClipboardList, show: true },
                    ...
                    <div className="card-premium" style={{ padding: '1.75rem', background: 'white' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                                <div style={{ background: '#f8fafc', padding: '0.4rem 1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)' }}>
                                    <span style={{ fontSize: '0.7rem', fontWeight: 900, color: 'var(--text-muted)' }}>TOTAL SESSIONS: </span>
                                    <span style={{ fontSize: '0.85rem', fontWeight: 800 }}>{patient.treatments?.length || 0}</span>
                                </div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                        <tr style={{ borderBottom: '2px solid var(--border-subtle)', textAlign: 'left' }}>
                                            ...
                                            <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: 'var(--text-muted)', textAlign: 'right' }}>ACTION</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {invoices.map(inv => (
                                            ...
                                            >
                                                ...
                                                <td style={{ padding: '1rem', fontWeight: 800 }}>
                                                    ...
                                                    <div style={{ fontSize: '0.65rem', fontWeight: 700, color: (inv.balanceAmount || 0) > 0 ? '#fb923c' : '#10b981' }}>
                                                        {(inv.balanceAmount || 0) > 0 ? `BAL: ₹${inv.balanceAmount.toLocaleString()}` : 'PAID IN FULL'}
                                                    </div>
                                                </td>
                                                <td style={{ padding: '1rem' }}>
                                                    <span style={{
                                                        ...
                                                        fontSize: '0.65rem',
                                                        fontWeight: 900,
                                                        background: inv.status === 'Paid' ? '#dcfce7' : inv.status === 'Unpaid' ? '#fee2e2' : '#ffedd5',
                                                        color: inv.status === 'Paid' ? '#10b981' : inv.status === 'Unpaid' ? '#ef4444' : '#9a3412'
                                                    }}>
